...
export function encryptRSA(plaintext: string, loginKey: NodeRSA.Key): string {
  const key = new NodeRSA();
  key.importKey(loginKey, "pkcs8-public");
...
export function getEncryptedVote(
  ...
  public_key: string
    ...
    const loginKey = key.importKey(public_key, "pkcs8-public");
